...
const ProductCard = ({ title, imageUrl, buttonText }) => {
  return (<div
    className="relative flex items-center justify-center w-full h-full p-20 bg-cover bg-center bg-[#E2E3E6]"
    // style={{ backgroundImage: `url(${imageUrl})` }}
  >
    <div className="absolute h-[90%] w-[95%]">
        <img className='object-cover h-full w-full' src={`${imageUrl}`} alt="" />
    </div>
    <div className="relative z-10 text-center text-white">
      <h2 className="text-8xl font-normal font-custom mb-4">{title}</h2>
